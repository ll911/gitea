...
ARG dep="asciidoctor freetype freetype-dev gcc g++ libpng libffi-dev py-pip python3-dev py3-pip py3-setuptools py3-docutils"
USER root 
WORKDIR /data
RUN apk update && apk --no-cache add ${dep} \
 && apk upgrade \
 && pip3 install --no-cache-dir jupyter pandoc \
